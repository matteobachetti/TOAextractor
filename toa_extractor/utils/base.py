...
KNOWN_OFFSETS = {"hxmt/le": -864 * u.us}
...
def process_and_copy_image(source_image_path, target_image_path, image_config):
    ...
    If the source and target paths are the same, no action is taken.
    If the target image already exists and is newer than the source image,
    no action is taken as well.
